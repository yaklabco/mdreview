 ...
 *
 * Platform-agnostic version: uses MessagingAdapter instead of chrome.runtime.sendMessage.
 * When no adapter is provided, caching is silently skipped (graceful degradation).
 */
...
// ---------------------------------------------------------------------------
// No-op debug logger (temporary until debug-logger is extracted to core)
// ---------------------------------------------------------------------------
const debug = {
  info: (..._args: unknown[]) => {},
  debug: (..._args: unknown[]) => {},
...
export class RenderPipeline {
  ...
  async render(options: RenderOptions): Promise<void> {
    ...
    try {
      ...
      // Cache the result if enabled and adapter is available
      if (useCache && filePath && this.messaging) {
        try {
          const cacheKey = await this.getCacheKey(
            filePath,
  ...
  /**
   * Get cache key via messaging adapter
   */
  private async getCacheKey(
    ...
    const response: unknown = await this.messaging.send({
  ...
  /**
   * Get cached result via messaging adapter
   */
  ...
  /**
   * Set cached result via messaging adapter
   */
